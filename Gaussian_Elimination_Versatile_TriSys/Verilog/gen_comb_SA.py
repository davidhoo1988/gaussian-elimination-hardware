...
python2.7 gen_comb_SA.py -k 4 -l 4 > comb_SA.v
'''
...
for row in range(row_num):
   ...
   for col in range(col_num):
     ...
     assign op_in_{row}_{col} = mode & start ? 2'b01 : 2'b00;
     ...
     wire pivout_out_{row}_{col};""".format(row = row, col = col))
         ...
         print ("""     assign op_in_{row}_{col} = mode & start ? 2'b01 : 2'b00;
...
for i in range(0,col_num):
  if i == col_num-1:
    print (" wire result_col{0};".format(col_num-1))
  else:
    print (" reg [{j}:1] result_col{k};".format(j = col_num-1-i, k = i)) 

print ("")

print (" assign result_col{col} = data_out_{row}_{col};".format(col = col_num-1, row = row_num-1))
print ("")

print ("  always @(posedge clk) begin")
for i in range(0,col_num-1):
  for j in range(1,col_num-i):
    if j == 1:
...
print (" assign result = {" + ", ".join(["result_col{i}[{j}]".format(i = i, j = col_num-1-i) for i in range(0,col_num-1)]) + ", result_col{0}".format(col_num-1) + "};")
